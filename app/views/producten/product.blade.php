...
Dankzij het groot aantal verschillende formaten beeldschermen is er ook voor jou in huis wel een geschikte televisie te vinden. Jij kiest namelijk zelf alle functies die jij tot je beschikking wilt hebben! Hierdoor kun jij ook gewoon genieten van de allerleukste televisieseries en films. Plezier beleef je zeker. Bestel jouw LED-televisie eenvoudig en snel! Toch nog even wat meer ontdekken? Dat kan door op de verschillende televisies te klikken. Hier vind je namelijk alle informatie die je nodig hebt!</p>

<ul>Product Namen:
	@foreach($producten as $prod)
			<li>{{ link_to("/producten/{$prod->id}", $prod->naam) }}</li>
	@endforeach
	{{dd($producten)}}
</ul>
